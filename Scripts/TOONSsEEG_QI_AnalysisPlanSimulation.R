...
d_sim$document_id <- 1:nrow(d_sim)
...
topic_numbers <- as.numeric(2:8) # models with a reasonable/interpretable range of topics
...
#-------------------------------------------------------------------------------
# Choose a model
topic_model <- models$'2'  # manually select model based on number of topics

plot_columns <- 2 # the number of columns in the grid of word clouds
...
combined_data <- merge(d_sim, doc_topic_scores, by = "document_id", all.x = TRUE)
