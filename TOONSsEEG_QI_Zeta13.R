...
N <- 2000  # number of patients for whom to simulate data
...
# This type of plot (particularly the PI "shade") doesn't really make sense with
# a categorical (indexed) predictor. However it is useful to compare it to the
# prior predictive simulation and to highlight the intercept and treatment
# slope. That said, what we're really interested in is the Post-Pre difference
# scores.
...
# Finally, let's do a posterior predictive check to assess parameter recovery.
...
# You can see that there is shrinkage toward zero
...
m0.2 <- ulam(
  alist(
    ...
    b[PP] ~ dnorm( 0 , 10 ),
    cutpoints ~ dnorm( 0 , 1.5 )
  ) , data=dat , chains=4 , cores=4 , iter=1e4)
...
## R code 11.8
dens(p)
dens( abs( p[,2] - p[,1] )  ) # implied absolute difference (Post-Pre)
...
dens(s[,2], lwd=3, col=rangi2, add=TRUE) # this is opposite of what's expected. Something weird going on...
...
plot(precis(mK0.0, 2, pars=c("b", "a_bar", "sigma_a"))) # parameter estimates on the logit (log-odds) scale
precis(mK0.0, 2, pars=c("b", "a_bar", "sigma_a"))

## R code 12.29
kPP <- 1:2   # values of PrePost to calculate over
pdat <- data.frame(PrePost=kPP)
s <- sim( mK0.0 , data=pdat )
dens(s[,1], lwd=3, xlab="Knowledge Need Met")
...
# dtm <- DocumentTermMatrix(corpus, control=list(stemming=TRUE)) # use stemming?

# Set the number of topics you want to find
num_topics <- 2

# Run LDA or CTM
# lda_model <- LDA(dtm, k = num_topics, method="VEM", control = list(seed = 1234))
lda_model <- LDA(dtm, k = num_topics, method="Gibbs", control = list(seed = 1234))
ctm_model <- CTM(dtm, k = num_topics, control = list(seed = 1234))

# Choose a model
# topic_model <- lda_model
topic_model <- ctm_model

# Get the top terms associated with each topic
terms(topic_model, 5)  # Show 5 terms for each topic

# Extract the topic-term probabilities
posterior_probs <- posterior(topic_model)
topic_term_probs <- posterior_probs$terms  # Topic-term probabilities
term_probs_1 <- t(topic_term_probs)[,1]
term_probs_2 <- t(topic_term_probs)[,2]

# Create a word-frequency matrix
word_freqs_1 <- data.frame(word = names(term_probs_1), freq = term_probs_1)
word_freqs_2 <- data.frame(word = names(term_probs_2), freq = term_probs_2)
...
wordcloud(words = word_freqs_1$word,
          freq = word_freqs_1$freq,
          min.freq = 1,
          max.words = 100,
          random.order = FALSE,
          rot.per = 0.2,
...
wordcloud(words = word_freqs_2$word,
          freq = word_freqs_2$freq,
          min.freq = 1,
          max.words = 100,
          random.order = FALSE,
          rot.per = 0.2,
...
par(mfrow=c(1,1)) # plots in 1 rows, 1 column
...
# I will tweak the cutoff based on the median for this corpus of ~1.2
...
# default. This indicates that in this case the Dirichlet distribution has more
# mass at the corners and hence, documents consist only of few topics.

# The entropy measure can also be used to indicate how the topic distributions
# diﬀer for the four ﬁtting methods. We determine the mean entropy for each
...
# over the topics. Looks like the Gibbs provides highest measure of entropy,
# which may indicate a greater diversity or uncertainty in topic distribution,
# whereas the VEM model provides the lowest entropy -- this may suggest that the
# topics are more focused, and defined by fewer words.
...
#-------------------------------------------------------------------------------
# Choose a model
topic_model <- models$'10'  # manually select model based on number of topics
...
wordcloud_plots <- lapply(1:num_topics, function(i) {
  ...
  ggplot(data = top_words, aes(label = word, size = freq, color = freq)) +
    ...
    theme(
      plot.background = element_rect(fill = "black"),  # Dark background for the plot
      panel.background = element_rect(fill = "black"),  # Dark background for the panel
...
do.call(grid.arrange, c(wordcloud_plots, ncol = 5))
#-------------------------------------------------------------------------------
# Save plot
# Set the output file name and dimensions
jpeg("./Plots/wordclouds.jpg", 
     width = 1280, height = 720, # tweak dimensions based on number of topics/words
     quality = 100)

# Use `grid.arrange` to arrange word clouds
do.call(grid.arrange, c(wordcloud_plots, ncol = 5))
